...
import { runGuardianAutoFix } from "../agent/guardian";
...
export function registerHandlers(bot: Telegraf) {
  ...
  bot.action(/^guardian_fix:(.+)$/, async (ctx) => {
    if (!isOwner(ctx.chat!.id)) return ctx.answerCbQuery();
    const filePath = ctx.match[1]!;
    await ctx.answerCbQuery('Starting AI auto-fix...');
    await ctx.reply(`🔧 [Guardian Co-Pilot] Attempting to auto-fix errors in \`${filePath}\`...`);
    
    void (async () => {
      const success = await runGuardianAutoFix(filePath);
      if (success) {
        await ctx.reply(`✅ Successfully auto-fixed compile issues in \`${filePath}\`!`);
      } else {
        await ctx.reply(`❌ Auto-fix failed or could not find a clean resolution.`);
      }
    })().catch(console.error);
  });
